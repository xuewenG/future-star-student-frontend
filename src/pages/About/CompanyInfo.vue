...
export default {
  ...
  onLoad (student) {
    ...
    SchoolmateInfoRequest.getSchoolmateInfo(uni.getStorageSync('student_id')).then(student => {
      ...
      this.companyForm = this.student.companyInfo
      for (let i = 0, lenI = this.positionList.length; i < lenI; ++i) {
        if (this.companyForm.positions.includes(this.positionList[i].value)) {
          this.$set(this.positionList[i], 'checked', true)
        }
      }
      let found = false
      for (let i = 0; i < this.financeList.length; i++) {
        console.log('finance', this.companyForm.finance, 'item', this.financeList[i])
        if (this.companyForm.finance === this.financeList[i]) {
          this.finance = this.companyForm.finance
          found = true
        }
      }
      if (!found) {
        this.finance = '其他'
        this.otherFinance = this.companyForm.finance
      }
      console.log('finance', this.finance)
      console.log('otherFinance', this.otherFinance)
      // if (typeof (this.student.staffNum) !== 'undefined') {
      //   console.log(this.companyForm)
      // }
    })
  ...
  methods: {
    ...
    saveChanges () {
      ...
      this.student.companyInfo.finance = this.finance
      console.log('companyInfo', this.student.companyInfo)
